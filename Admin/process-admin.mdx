...
sidebar-title: "Process Administration"
icon: "arrow-progress"
---
...
- The process is made up of a series of **Tasks**, which may be guided by **Task Rules**.
- Items pass through the process using the **Connectors** to move through the tasks. Connectors are specific actions that can change the status (and **Stages**) of the item as they move it between task steps.
...
<Steps>
  ...
  <Step title="Define Tasks and Rules">
    In your process flow, fill in the details of each task and all of the rules that govern it.
    ...
    As you build your tasks and rules, group them into the appropriate Stages.
  </Step>
  ...
  <Step title="Define How The Tasks Connect">
    Each task must have an input and an output connection. The connectors define the actions that will be taken to move the item between this task and its next one. Define each connector and what it does.

    > For example: if a connector reads an incoming email and passes it to a validator, it’s likely an OCR connector that extracts specific fields.
  </Step>
...
7. Next, specify each of the fields in your data file, along with their “friendly” name and type. 
   - **Field Name** should exactly match the column header in the data file. 
   ...
   - Click **Hidden** if the field should not be displayed in Actionabl.
...
- A file the user is prompted to upload whenever the data source is called
...
- **Static Form**, **Dynamic Form**, and **Custom Form** task actions are used in tandem with the Local Actions in a task to send a form to the user to fill out. 
  - Static forms are pre-created and standard forms that the process sends, such as a password reset or a standardized request for information.
  - Dynamic forms are created on the fly based on the unique information in the task. For example, this could send a request for more information based on certain criteria found in an invoice’s data.
  - Custom forms **<u>(NOT DEFINED IN TASK MANAGEMENT GUIDE - NEED MORE INFO)</u>**
...
While tasks have internal actions, connectors are the pathways and actions that move an item between tasks. There are 6 categories of connectors in Actionabl, each of which is covered in detail below:
...
- Select a **custom task icon** for this connector, which is used in process modeling and anywhere else that the connector is visually depicted. Your system administrator can upload additional icons to the ?????? folder.
- Check the **Active** box to make this connector usable and visible in process modeling.

### Start Event Connectors

The Start Event connector represents the action that starts a process.
...
The folders that can be monitored must exist in the ?????? path on the server.
...
- **Save as Attachment** indicates that when the file is found, it should be saved in Actionabl as an attachment to the item created. If this is checked:
...
The FTP start event monitors the server for inbound FTP actions and starts the process.

#### **Custom Start Event**

A Custom start event executes the specific code script specified using the event's parameters. The script's output is used as input to the first task in the process.
...
As with Custom Start events - what triggers the script?
...
- You can designate it as Machine Only which ????
...
- **Email Template** is the template to use for the email’s contents. These templates usually include variables, representing the data passed into the process step.
...
- **Prepared Email Body Field** is what????
...
You can expand any of the Email Body sections to see the actual text and variables. Items in **{{ }}** represent variables.
