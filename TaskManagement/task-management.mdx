...
<AccordionGroup>
  <Accordion title="The Process/Task/Stage Structure" description="Learn how the elements of a process come together and get organized into tasks" icon="arrow-progress">
    ...
    **_Tasks_**
    ...
    | :------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------------------------------------ |
    | **Decision**              | Assess whether a condition has been met, like checking whether an email has been received or if a value matches a criterion.                                                                                                          | ![DM Sv](/images/ICONS/SVGs/DM.svg)                     |
    | **User**                  | This task must always be completed by a human, like reviewing a proof or shipping a product. Your company may also define some types of errors to be user tasks, such as resolving an invoice that doesn’t match the expected amount. | ![UR Sv](/images/ICONS/SVGs/UR.svg)                     |
  ...
  <Accordion title="Organizing Your Tasks" description="Learn about how tasks are grouped and move through processes" icon="sitemap">
    ...
    **_Building a complex query_**
    ...
    | **Equals ignore case**   | The full data in the field exactly matches the search string, including leading/trailing spaces. However, the capitalization of the content is ignored.                                                                                                                               | Civil          | _civil_                                                                            |
    ...
    **_Query Grouping (Grouped Conditions)_**
    ...
    - Originate with vendors A or B and are greater than $$1000 OR that originate with vendors C or H and are greater than $$
...
1. The task detail screen opens for you to take action, and the item is now listed in your My Tasks module on the dashboard. You now own this task.
...
7. When you are finished:
   1. Resolve the item and move it to the next process task by clicking **Complete**.
   2. If appropriate, you can **Discard** the item, which removes it from the process. The item is still found by Active Search with a status of “discarded.” (Click **\<\< MORE** and then **Discard**)
...
2. Click on **Content & History** at the bottom of the screen and review any **Notes** and **Attachments**, as well as **Emails** relating to the item.
...
Business users may receive certain types of Error Tasks. These are usually data-related errors, such as missing or invalid data. They can also be problems like a duplicate invoice or missing document attachment. 
...
7. When you are finished:
   1. Resolve the item:
      1. **Resubmit** it to send it back through this automated step with the corrections you’ve made to it. This will <u>revalidate</u> and <u>reprocess</u> the step completely.
      2. <u>Bypass</u> automated re-processing and move it to the next process task by clicking **Complete**. This indicates that you have verified that all of the processes within this step have been successfully completed, manually or otherwise. 
...
- Each note should be <u>specific</u> to a topic <u>and</u> audience, as you can have multiple notes attached to an item as it moves through the workflow. For example, you could have one for the accounting team and another for logistics.
...
3. Click the green plus icon  on the right of the screen to add a note. The note editor appears as a pop-up.\
...
4. Use the note editor like any text editor, such as Word or Docs. 
   1. Shortcuts like Copy (Ctrl/Cmd C) and Paste (Ctrl/Cmd V) work here.
   2. The Insert menu lets you easily add images, links, horizontal lines, and tables.
   3. Formatting options across the top of the screen provide bold, italic, font color, background color, alignment, and list/indent styles.
   4. The Edit menu provides Undo / Redo and Select All options, in addition to the usual cut, copy, and paste.
...
7. Now, adjust the note’s name and access level.
   1. Click the expand icon on the far right of the item to open its edit menu.\
      ![TMB57 Updatenote Jp](/images/TaskManagement/TMB57-Updatenote.jpg)
   2. If you need to make changes to the content you just saved, click **Edit Note**.
   3. To update the note’s name, access level, and task note status, click **Edit**.\
      ![TMB58 Noteupdate Jp](/images/TaskManagement/TMB58-Noteupdate.jpg)
   4. Type a descriptive name for the note. 
   5. Click the down arrow under Access Level and select the appropriate security level. 
      1. **User** restricts the note and its content to be visible to only you.
      2. **Group** allows anyone in the same user group as you to see the note and its content.
      3. **Public** lets anyone who can see this item see the note and its content.
   6. Indicate if this is a \*\*Local \*\*note by clicking the checkbox. Local notes are only seen on this item during this processing step; when the task is resolved, the note disappears. If you leave this unclicked, the note will remain on the item throughout its life, regardless of its step or status.
...
<Tip>
  **NOTE**: When an item is discarded, it is still found via Active search, with a status of “Discarded.”
</Tip>

## **Bulk actions**

There are a number of actions you can perform simultaneously on a group of selected items. This is called a **Bulk Action**.
