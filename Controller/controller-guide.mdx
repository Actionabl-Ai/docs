...
<Steps>
  ...
  <Step title="Change Automation Configuration">
    ...
    3. All automations require a **Connector**. Click the selection icon to pick the connector from the list.
       ...
       <Tip>
         **HINT**: Configure connectors on the Task Manager dashboard (for users with technical, supervisory, or system admin rights). Details are in the **Task Manager** guide (COMING SOON).
       </Tip>
  </Step>
  ...
  <Step title="Set up a Recurrent Schedule or Cron" stepNumber="5">
    ...
    2. Enter a **Recurrence Frequency** (in milliseconds). This is the wait time between the <u>end</u> of one execution and the <u>start</u> of the next.
