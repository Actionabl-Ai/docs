...
- **Task Management** (COMING SOON) covers all aspects of the Task Manager dashboard, which includes:
...
<img src="/images/ControllerGuide/C1-Controllermenu.png" width="30%">
...
<Steps>
  ...
  <Step title="Set up a Recurrent Schedule or Cron" stepNumber="5">
    ...
    5. **Updated By** and **Updated At** show the user who last edited this automation and when they were done.
  </Step>
</Steps>
...
2. The existing rules are displayed in the list. To view their details, click on the expand icon to the right of any rule.
...
When the Actionabl team sets up your system, the AI spaces are also set up as part of that process, so client users cannot edit. 
...
Cron jobs are tasks that are managed by a time-based scheduler that is built into all Unix-based operating systems. You use a cron schedule to specify precisely when and with what frequency the task or script should execute, like every 15 minutes or every other Thursday at 2 PM.
...
The **Type**  is either _Report_, _Batch Job_, or_Task Event_ . When you pick one, the appropriate additional fields appear to set it up.
...
1. Click on the browse brackets in the **Report Definition** field to select the report script to schedule. 
2. **Email template** can be left blank to use the default in the system. Alternatively, click the browse brackets to select a different template.

![CG59 Cron Reports Completed Jp](/images/ControllerGuide/CG59-CronReportsCompleted.jpg)

## **_Batch Job Cron Setup_**

1. Specify a unique **Name** for the cron. This should be informative when browsing the list of all crons. For example, bank_invoice_process_run_daily_2am
...
1. Click on the browse brackets in the **Batch Job Script** field to select the script to schedule. 
...
1. Click the browse brackets in the **Cron Schedule** field, then select which of the available schedules you wish for this job. You can add a new schedule by clicking the green plus icon  at the top right of the screen.
...
2. Enter a **unique ID** for the schedule. The ID must contain only letters and numbers, underscores, and dashes. It cannot contain any spaces.

![CG61 Cron Schedulenew1 Jp](/images/ControllerGuide/CG61-CronSchedulenew1.jpg)

1. Type a **Name** for this schedule. The name will appear to users on the list of schedules.
2. Ensure that **Active** is checked.
3. Click the plus sign on the right of the cron schedules to start setting up the parameters.
...
- Click the browse brackets to specify **Disable Rule(s)** that disqualify items from being included in this cron job. See [**_<u>Configuring Disable Rules for Automations</u>_**](https://docs.google.com/document/d/1P-TO70rqYfM1HmydmtE3XgnMgwAzoAgutgInby3bnio/edit?tab=t.z38ggzyxt3cf#heading=h.drhvuiacy6wr) for details about defining rules.
